...
var ManualModel = BasicModel.extend({
    avoidCreate: false,
    quickCreateFields: ['account_id', 'journal_id', 'amount', 'department_id', 'division_id', 'project_id', 'branch_id','analytic_account_id', 'name', 'tax_ids', 'force_tax_included', 'date', 'to_check'],
    ...
    **/
    ...
    _formatToProcessReconciliation: function (line, prop) {
        ...
        if (prop.department_id) result.department_id = prop.department_id.id;
        if (prop.division_id) result.division_id = prop.division_id.id;
        if (prop.branch_id) result.branch_id = prop.branch_id.id;
        if (prop.project_id) result.project_id = prop.project_id.id;        
    ...
    _formatQuickCreate: function (line, values) {
        ...
        var prop = {
            ...
            'project_id': this._formatNameGet(values.project_id),
            'division_id': this._formatNameGet(values.division_id),
            'department_id': this._formatNameGet(values.department_id),
            'branch_id': this._formatNameGet(values.branch_id),
