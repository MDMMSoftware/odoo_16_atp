...
var ManualLineRenderer = Widget.extend(FieldManagerMixin, {
    ...
    _renderCreate: function (state) {
        ...
        return this.model.makeRecord('account.bank.statement.line', [{
            ...
            relation: 'analytic.project.code',
            type: 'many2one',
            name: 'project_id',
            domain: ["|", ['company_id', '=', state.st_line.company_id], ['company_id', '=', false]],            
        }, {
            ...
            self.fields.project_id = new relational_fields.FieldMany2One(self,
                'project_id', record, {mode: 'edit', attrs: {can_create:false}});

            self.fields.date = new basic_fields.FieldDate(self,
                'date', record, {mode: 'edit'});

            self.fields.division_id = new relational_fields.FieldMany2One(self,
                'division_id', record, {mode: 'edit', attrs: {can_create:false}});                

            self.fields.to_check = new basic_fields.FieldBoolean(self,
                'to_check', record, {mode: 'edit'});
            
            self.fields.department_id = new relational_fields.FieldMany2One(self,
                'department_id', record, {mode: 'edit', attrs: {can_create:false}});                
    
            self.fields.branch_id = new relational_fields.FieldMany2One(self,
                'branch_id', record, {mode: 'edit', attrs: {can_create:false}});                 
            ...
            self.fields.department_id.appendTo($create.find('.create_department_id .o_td_field'));
            self.fields.date.appendTo($create.find('.create_date .o_td_field'));
            self.fields.project_id.appendTo($create.find('.create_project_id .o_td_field'));            
            self.fields.to_check.appendTo($create.find('.create_to_check .o_td_field'));
            self.fields.division_id.appendTo($create.find('.create_division_id .o_td_field')); 
            self.fields.branch_id.appendTo($create.find('.create_branch_id .o_td_field'));                         
