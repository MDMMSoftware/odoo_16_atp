...
class AccountMoveLine(models.Model):
    ...
    def _prepare_reconciliation_single_partial(self, debit_vals, credit_vals):
        ...
        elif  debit_vals['currency'] != credit_vals['currency'] and not has_debit_zero_residual and not has_credit_zero_residual:
            # this is customized code block not to create exchange moves when differenct currencies
            # Everything will be expressed in company's currency  so that there won't be no exchange journals and there is something left to reconcile.
            recon_currency = company_currency     
            debit_rate = credit_rate = 1.0   
            if  debit_vals['currency'] == company_currency and credit_vals['currency'] != company_currency:
                recon_debit_amount = recon_currency.round(remaining_debit_amount * debit_rate)
                recon_credit_amount = ( -remaining_credit_amount_curr ) / get_accounting_rate(credit_vals)
                temp_rate = get_accounting_rate(credit_vals)
            elif debit_vals['currency'] != company_currency and credit_vals['currency'] == company_currency:
                recon_debit_amount = remaining_debit_amount_curr / get_accounting_rate(debit_vals)
                recon_credit_amount = recon_currency.round(-remaining_credit_amount * credit_rate)
                temp_rate = get_accounting_rate(debit_vals)
            elif debit_vals['currency'] != company_currency and credit_vals['currency'] != company_currency:
                recon_currency = company_currency
                ...
                debit_rate = credit_rate = temp_rate = 1.0
                recon_debit_amount = remaining_debit_amount
                recon_credit_amount = -remaining_credit_amount       
        ...
        if recon_currency == company_currency:
            ...
            if debit_rate:
                ...
                if temp_rate and debit_vals['currency'] != company_currency:
                    partial_debit_amount_currency = partial_debit_amount_currency * temp_rate
            ...
            if credit_rate:
                ...
                if temp_rate and credit_vals['currency'] != company_currency:                
                    partial_credit_amount_currency = partial_credit_amount_currency * temp_rate
