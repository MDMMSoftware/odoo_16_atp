...
export class RepairTimer extends Component {
    setup() {
        ...
        onWillStart(async () => {
            if(this.props.ongoing === undefined && !this.props.record.model.useSampleModel && this.props.record.data.state == "progress") {
                const additionalDuration = await this.orm.call('job.rate.detail', 'get_working_duration', [this.props.record.resId]);
...
RepairTimer.supportedTypes = ["float"];
RepairTimer.template = "repair_external.RepairTimer";

registry.category("fields").add("repair_timer", RepairTimer);
registry.category("formatters").add("repair_timer", formatMinutes);