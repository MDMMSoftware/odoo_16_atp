...
class AccountAsset(models.Model):
    ...
    code = fields.Char()
    asset_code_prefix_id = fields.Many2one(string="Prefix",comodel_name='asset.code.prefix')
    ...
    group_type_id = fields.Many2one("asset.group.type" ,string="Group Type")
    category_id = fields.Many2one("asset.category" ,string="Category")
    type_id = fields.Many2one("asset.type" ,string="Type")
    brand_id = fields.Many2one("asset.brand" ,string="Brand")
    access_model_id = fields.Many2one("asset.model" ,string="Model")
    model_year = fields.Integer()
    color_id = fields.Many2one("asset.color" ,string="Color")
    serial_no = fields.Char()
    capacity_weight = fields.Char()
    user_id = fields.Many2one("res.users" ,string="User")
    department_id = fields.Many2one("res.department" ,string="Department",tracking=True)
    ...
    @api.onchange('asset_code_prefix_id')
    def auto_generate_asset_code(self):
        asset_sequence = self.env['asset.code.sequence']
        if self.asset_code_prefix_id:
            code_sequence = asset_sequence.search([('prefix','=',self.asset_code_prefix_id.id)])

            if code_sequence:
                seq = str(code_sequence.seq)
                if len(seq) > self.asset_code_prefix_id.padding:
                    raise ValidationError("Sequence exceeded the defined padding!!")
                self.seq = seq.zfill(self.asset_code_prefix_id.padding)
            else:
                asset_sequence.create({
                    'prefix':self.asset_code_prefix_id.id,
                    'seq':1
                })
                self.seq = "1".zfill(self.asset_code_prefix_id.padding)
        # else:
        #     raise ValidationError("Please choose Partner Prefix first to generate code")

    @api.constrains('asset_code_prefix_id')
    def save_asset_code(self):
        asset_prefix = self.asset_code_prefix_id
        if asset_prefix:
            code_sequence = self.env['asset.code.sequence'].search([('prefix','=',asset_prefix.id)],limit=1)
            if code_sequence and self.seq:
                if int(self.seq) == code_sequence.seq:
                    self.code = f"{asset_prefix.name}-{self.seq}"
                    code_sequence.seq += 1
            else:
...
class AssetGroupType(models.Model):
    _name = "asset.group.type"
...
class AssetCodeSequence(models.Model):
    _name = 'asset.code.sequence'
    _order = "prefix"

    prefix = fields.Many2one(string="Prefix",comodel_name="asset.code.prefix")
    seq = fields.Integer(string="Next Sequence",default=1)

class AssetCodePrefix(models.Model):
    _name = "asset.code.prefix"

    name = fields.Char()
    padding = fields.Integer(max=7,min=3)

    _sql_constraints = [
        ('uniq_asset_code_prefix', 'unique(name)', 'Asset Code Prefix must be unique.'),
    ]       
