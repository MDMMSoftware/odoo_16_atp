...
class AccountPayment(models.Model):
    ...
    def action_open_reconcilation_entry(self):
        return {
            'name': _('Reconcilation Entry'),
            'view_mode': 'tree,form',
            'res_model': 'account.move',
            'type': 'ir.actions.act_window',  
            'domain': [('id', 'in',self.reconcilation_move_ids.ids)],              
        }
    ...
    @api.onchange("partner_type")
    def _add_domain_partner_id_by_partner_type(self):
        for res in self:
            if res.partner_type == 'customer':
                return {"domain":{"partner_id" : [('partner_type','=','customer')]}}
            elif res.partner_type == 'supplier':
                return {"domain":{"partner_id" : [('partner_type','=','vendor')]}}
