...
class GeneralLedgerCustomHandlerInherit(models.AbstractModel):
    _name = 'account.general.ledger.report.handler.inherit'
    _inherit = 'account.report.custom.handler'
    _description = 'General Ledger Custom Handler Inherit'
    ...
    def _custom_options_initializer(self, report, options, previous_options=None):
        ...
        desired_list = ['Date','Project','Account Code','Account Name','Label','Partner','Currency','Debit','Credit','Balance'] # start of addition
        result = []
        for list in desired_list:
            for res in options['columns']:
                if res['name']==list:
                    result.append(res)
        options['columns'] = result # end of addition
    ...
    def _custom_unfold_all_batch_data_generator(self, report, options, lines_to_expand_by_function):
        ...
        for line_dict in lines_to_expand_by_function.get('_report_expand_unfoldable_line_general_ledger_inherit', []):
    ...
    def _get_aml_values(self, report, options, expanded_account_ids,model_id=None, offset=0, limit=None):
        # expanded_account_ids = expanded_account_ids[0]    # start of addition
        rslt = {account_id: {} for account_id in expanded_account_ids}
        aml_query, aml_params = self._get_query_amls(report, options, expanded_account_ids,model_id, offset=offset, limit=limit)
        self._cr.execute(aml_query, aml_params)
        aml_results_number = 0
        has_more = False
        result = self._cr.dictfetchall() #  start of addition
        for aml_result in result:
            ...
            # account_result = rslt # start of addition
    ...
    def _get_query_amls(self, report, options, expanded_account_ids,model_id, offset=0, limit=None):
        ...
        for column_group_key, group_options in report._split_options_per_column_group(options).items():
            ...
            where_params.append(tuple(model_id))    # start of addition
            where_params.append(tuple(model_id))
            ct_query = self.env['res.currency']._get_query_currency_table(group_options)
            where_clause = where_clause+"""AND ("account_move_line"."id" in (select id from account_move_line where move_id in 
                            (select am.id from account_move am 
                            LEFT JOIN account_move_line aml on aml.move_id=am.id
                            where aml.account_id in %s) and account_id not in %s))""" # end of addition
            query = f'''
                (SELECT
                    ...
                    ROUND(account_move_line.debit * currency_table.rate, currency_table.precision)   AS credit,
                    ROUND(account_move_line.credit * currency_table.rate, currency_table.precision)  AS debit,
    ...
    def _get_account_title_line(self, report, options, account, has_lines, eval_dict):
        ...
        return {
            ...
            'expand_function': '_report_expand_unfoldable_line_general_ledger_inherit',
    ...
    def _get_aml_line(self, report, parent_line_id, options, eval_dict, init_bal_by_col_group):
        ...
        for column in options['columns']:
            ...
            if col_value is None:
                ...
                elif col_expr_label == 'communication' or col_expr_label == 'partner_name' or col_expr_label == 'name' or col_expr_label == 'project': # end of addition - 18
    ...
    def _report_expand_unfoldable_line_general_ledger_inherit(self, line_dict_id, groupby, options, progress, offset, unfold_all_batch_data=None):
        ...
        report = self.env.ref('account_reports.general_ledger_report_inherit')  # # start of addition - 18
        ...
        if unfold_all_batch_data:
            ...
            result = [] # start of addition - 20
            res = False
            sql = """select account_id from account_move_line where move_id in 
                (select am.id from account_move am 
                LEFT JOIN account_move_line aml on aml.move_id=am.id
                where aml.account_id = %s) and account_id != %s order by date"""
            self.env.cr.execute(sql,(model_id,model_id,))
            res = self.env.cr.fetchall()
            if res:
                total_balance =0
                if type(progress)==float or type(progress)==int:
                    next_progress = progress
                elif type(progress)==str:
                    next_progress = float(progress)
                else:
                    next_progress = list(progress.values())[0]
                final_res = []
                for val in list(set(res)):
                    result.append(val[0])
                    aml_results, has_more = self._get_aml_values(report, options, [val[0]],[model_id], offset=offset, limit=limit_to_load)
                    aml_results = aml_results[val[0]]
                    for aml in aml_results.items():
                        final_res.append(aml)

                    
                for aml_results in sorted(final_res,key=lambda x:x[0][1]):
                    # for aml_result in aml_results[1]:
                    balance_dict = list(aml_results[1].values())[0]
                    if balance_dict.get('balance')>0:
                        balance = -1 * balance_dict.get('balance')
                    else:
                        balance = abs(balance_dict.get('balance'))
                    # total_balance += balance
                    new_line = self._get_aml_line(report, line_dict_id, options, aml_results[1], next_progress+balance)
                    lines.append(new_line)
                    next_progress = next_progress+balance
                    init_load_more_progress(new_line)
        has_more = False
        if len(lines)>0:
            has_more = True
            
        # if options['date']['date_from']!=options['date']['date_to']:
        #     lines = sorted(lines, key=lambda f: f['columns'][0]['name'])
        return {
            'lines': lines,
