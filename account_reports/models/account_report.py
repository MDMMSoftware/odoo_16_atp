...
class AccountReport(models.Model):
    ...
    filter_project = fields.Boolean(
        string="Projects",
        compute=lambda x: x._compute_report_option_filter('filter_project'), readonly=False, store=True, depends=['root_report_id'],
    )
    filter_division = fields.Boolean(
        string="Divisions",
        compute=lambda x: x._compute_report_option_filter('filter_division'), readonly=False, store=True, depends=['root_report_id'],
    )
    filter_fleet = fields.Boolean(
        string="Fleets",
        compute=lambda x: x._compute_report_option_filter('filter_fleet'), readonly=False, store=True, depends=['root_report_id'],
    ) 
    
    filter_job_code = fields.Boolean(
        string="Job Codes",
        compute=lambda x: x._compute_report_option_filter('filter_job_code'), readonly=False, store=True, depends=['root_report_id'],
    )    
    ...
    def _init_options_project(self, options, previous_options=None):
        ...
        previous_project_ids = previous_options and previous_options.get('project_ids') or []

        selected_project_ids = [int(project) for project in previous_project_ids]
        # search instead of browse so that record rules apply and filter out the ones the user does not have access to
        selected_projects = selected_project_ids and self.env['analytic.project.code'].with_context(active_test=False).search([('id', 'in', selected_project_ids)]) or self.env['res.partner']
    ...
    def _init_options_division(self, options, previous_options=None):
        if not self.filter_division:
            return

        options['division'] = True
        previous_division_ids = previous_options and previous_options.get('division_ids') or []

        selected_division_ids = [int(division) for division in previous_division_ids]
        # search instead of browse so that record rules apply and filter out the ones the user does not have access to
        selected_divisions = selected_division_ids and self.env['analytic.division'].with_context(active_test=False).search([('id', 'in', selected_division_ids)]) or self.env['res.partner']
        options['selected_division_ids'] = selected_divisions.mapped('name')
        options['division_ids'] = selected_divisions.ids

        

    @api.model
    def _get_options_division_domain(self, options):
        domain = []
        if options.get('division_ids'):
            division_ids = [int(division) for division in options['division_ids']]
            domain.append(('division_id', 'in', division_ids))
    ...
    def _init_options_fleet(self, options, previous_options=None):
        if not self.filter_fleet:
            return

        options['fleet'] = True
        previous_fleet_ids = previous_options and previous_options.get('fleet_ids') or []

        selected_fleet_ids = [int(fleet) for fleet in previous_fleet_ids]
        # search instead of browse so that record rules apply and filter out the ones the user does not have access to
        selected_fleets = selected_fleet_ids and self.env['fleet.vehicle'].with_context(active_test=False).search([('id', 'in', selected_fleet_ids)]) or self.env['res.partner']
        options['selected_fleet_ids'] = selected_fleets.mapped('name')
        options['fleet_ids'] = selected_fleets.ids
    ...
    def _init_options_job_code(self, options, previous_options=None):
        if not self.filter_job_code:
            return

        options['job_code'] = True
        previous_job_code_ids = previous_options and previous_options.get('job_code_ids') or []

        selected_job_code_ids = [int(job_code) for job_code in previous_job_code_ids]
        # search instead of browse so that record rules apply and filter out the ones the user does not have access to
        selected_job_codes = selected_job_code_ids and self.env['job.code'].with_context(active_test=False).search([('id', 'in', selected_job_code_ids)]) or self.env['res.partner']
        options['selected_job_code_ids'] = selected_job_codes.mapped('name')
        options['job_code_ids'] = selected_job_codes.ids

        

    @api.model
    def _get_options_fleet_domain(self, options):
        domain = []
        if options.get('fleet_ids'):
            fleet_ids = [int(fleet) for fleet in options['fleet_ids']]
            domain.append(('fleet_id', 'in', fleet_ids))
       
        return domain
    
    @api.model
    def _get_options_job_code_domain(self, options):
        domain = []
        if options.get('job_code_ids'):
            job_code_ids = [int(job_code) for job_code in options['job_code_ids']]
            domain.append(('job_code_id', 'in', job_code_ids))
    ...
    def _init_options_account_type(self, options, previous_options=None):
        ...
        if self.name == 'Partner Ledger Inherit':
            options['account_type'] = [
                {'id': 'non_trade_receivable', 'name': _("Non Trade Receivable"), 'selected': True},
                {'id': 'non_trade_payable', 'name': _("Non Trade Payable"), 'selected': True},
            ]
            ...
            options['account_type'] = [
                {'id': 'all_id', 'name': _("All Account"), 'selected': True},
                {'id': 'asset_cash', 'name': _("Bank & Cash"), 'selected': False},
            ]
        ...
        check_if_name_applicable({'asset_cash'}, _("Bank & Cash"))
        check_if_name_applicable({'all_id'}, _("All Accounts"))
    ...
    def _get_options_account_type_domain(self, options):
        ...
        for opt in options.get('account_type', []):
            ...
            elif opt['id'] == 'asset_cash':
                domain = [ ('account_id.account_type', '=', 'asset_cash')]
            elif opt['id'] == 'all_id':
                domain = [ ('account_id.account_type', '!=', False)]
            if opt['selected']:
    ...
    def _get_options_domain(self, options, date_scope):
        ...
        domain += self._get_options_division_domain(options)
        domain += self._get_options_fleet_domain(options)
        domain += self._get_options_job_code_domain(options)
        domain += self._get_options_partner_domain(options)
