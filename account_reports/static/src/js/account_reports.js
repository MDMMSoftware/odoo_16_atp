...
var accountReportsWidget = AbstractAction.extend({
    ...
    custom_events: {
        ...
        'project_filter_changed': function(ev) {
            var self = this;
            self.report_options.project_ids = ev.data.project_ids;
            return self.reload().then(function () {
                self.$searchview_buttons.find('.account_project_filter').click();
            });
       },

       'division_filter_changed': function(ev) {
            var self = this;
            self.report_options.division_ids = ev.data.division_ids;
            return self.reload().then(function () {
                self.$searchview_buttons.find('.account_division_filter').click();
            });

        },    
        
        'fleet_filter_changed': function(ev) {
            var self = this;
            self.report_options.fleet_ids = ev.data.fleet_ids;
            return self.reload().then(function () {
                self.$searchview_buttons.find('.account_fleet_filter').click();
            });

        },   
        
        'job_code_filter_changed': function(ev) {
            var self = this;
            self.report_options.job_code_ids = ev.data.job_code_ids;
            return self.reload().then(function () {
                self.$searchview_buttons.find('.account_job_code_filter').click();
            });
    ...
    render_searchview_buttons: function() {
        ...
        // project filter
        if (this.report_options.project) {
            if (!this.projects_m2m_filter) {
                var fields = {};
                if ('project_ids' in this.report_options) {
                    fields['project_ids'] = {
                        label: _t('Projects'),
                        modelName: 'analytic.project.code',
                        value: this.report_options.project_ids.map(Number),
                    };
                }
                
                if (!_.isEmpty(fields)) {
                    this.projects_m2m_filter = new M2MFilters(this, fields, 'project_filter_changed');
                    this.projects_m2m_filter.appendTo(this.$searchview_buttons.find('.js_account_project_m2m'));
                }
            } else {
                this.$searchview_buttons.find('.js_account_project_m2m').append(this.projects_m2m_filter.$el);
            }
        }

        // division filter
        if (this.report_options.division) {
            if (!this.divisions_m2m_filter) {
                var fields = {};
                if ('division_ids' in this.report_options) {
                    fields['division_ids'] = {
                        label: _t('Divisions'),
                        modelName: 'analytic.division',
                        value: this.report_options.division_ids.map(Number),
                    };
                }
                
                if (!_.isEmpty(fields)) {
                    this.divisions_m2m_filter = new M2MFilters(this, fields, 'division_filter_changed');
                    this.divisions_m2m_filter.appendTo(this.$searchview_buttons.find('.js_account_division_m2m'));
                }
            } else {
                this.$searchview_buttons.find('.js_account_division_m2m').append(this.divisions_m2m_filter.$el);
            }
        }        

        // fleet filter
        if (this.report_options.fleet) {
            if (!this.fleets_m2m_filter) {
                var fields = {};
                if ('fleet_ids' in this.report_options) {
                    fields['fleet_ids'] = {
                        label: _t('Fleets'),
                        modelName: 'fleet.vehicle',
                        value: this.report_options.fleet_ids.map(Number),
                    };
                }
                
                if (!_.isEmpty(fields)) {
                    this.fleets_m2m_filter = new M2MFilters(this, fields, 'fleet_filter_changed');
                    this.fleets_m2m_filter.appendTo(this.$searchview_buttons.find('.js_account_fleet_m2m'));
                }
            } else {
                this.$searchview_buttons.find('.js_account_fleet_m2m').append(this.fleets_m2m_filter.$el);
            }
        }   


        // job_code filter
        if (this.report_options.job_code) {
            if (!this.job_codes_m2m_filter) {
                var fields = {};
                if ('job_code_ids' in this.report_options) {
                    fields['job_code_ids'] = {
                        label: _t('Job Codes'),
                        modelName: 'job.code',
                        value: this.report_options.job_code_ids.map(Number),
                    };
                }
                
                if (!_.isEmpty(fields)) {
                    this.job_codes_m2m_filter = new M2MFilters(this, fields, 'job_code_filter_changed');
                    this.job_codes_m2m_filter.appendTo(this.$searchview_buttons.find('.js_account_job_code_m2m'));
                }
            } else {
                this.$searchview_buttons.find('.js_account_job_code_m2m').append(this.job_codes_m2m_filter.$el);
            }
        }   
