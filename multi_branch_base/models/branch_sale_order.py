...
class SaleOrder(models.Model):
    ...
    def onchange_branch_id(self):
        ...
        if self.branch_id and self.branch_id not in user_object.branch_ids and user_object.branch_ids:
        ...
        if self.branch_id:
            ...
            if not warehouse:
                ...
                raise UserError(error_msg) 
            warehouse_id = user_object.with_company(self.company_id.id)._get_default_warehouse_id()
            location_id = user_object.with_company(self.company_id.id)._get_default_location_id()
            if warehouse_id and not self.warehouse_id:
                self.warehouse_id = warehouse_id
            if location_id and not self.location_id:
                self.location_id = location_id                      
        else:
            self.branch_id = user_object.branch_id
            warehouse_id = user_object.with_company(self.company_id.id)._get_default_warehouse_id()
            location_id = user_object.with_company(self.company_id.id)._get_default_location_id()
            if warehouse_id and not self.warehouse_id:
                self.warehouse_id = warehouse_id
            ...
            if not self.pricelist_id:
                self.pricelist_id = user_object.pricelist_id
