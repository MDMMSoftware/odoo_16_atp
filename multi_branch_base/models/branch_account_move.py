...
class AccountMove(models.Model):
    ...
    def onchange_branch_id(self):
        ...
        if self.expense_type=='advance_claim':
            ...
            self.journal_id = adv_claim_journal
        elif self.expense_type=='dir_exp':
            journal = self.env.user.payment_journal_id
            if not journal:
                journal = self.env['account.journal'].search([('type','=','cash')],limit=1,order='id asc')
            self.journal_id = journal and journal.id or self.journal_id
            self.journal_account = self.journal_id.default_account_id.id  
        else:      
